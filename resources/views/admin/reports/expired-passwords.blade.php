                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <a href="{{ route('admin.reports.users') }}"
                            class="inline-flex items-center gap-1 text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-200 text-sm transition">
                            ← Back to User Report
                        </a>
                    </div>

                    @if($users->isEmpty())
                        <div class="text-center py-12 text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-900/20 rounded-lg">
                    ...
                    @else
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-red-50 dark:bg-red-900/20">
                                    <tr>
                                        ...
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Password Expired</th>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Days Overdue</th>
                                ...
                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    @foreach($users as $user)
                                        <tr class="bg-red-50/50 dark:bg-red-900/10 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
                                            <td class="px-4 py-3 text-sm font-mono text-gray-700 dark:text-gray-300">{{ $user->username }}</td>
                                            <td class="px-4 py-3 text-sm font-medium text-gray-900 dark:text-gray-100">{{ $user->full_name }}</td>
                                            ...
                                            <td class="px-4 py-3 text-sm">
                                                ...
                                                <span class="px-2.5 py-1 rounded-full text-xs font-medium bg-{{ $sc }}-100 text-{{ $sc }}-800 dark:bg-{{ $sc }}-900/40 dark:text-{{ $sc }}-300 capitalize">
                                                    {{ $user->status }}
                                                </span>
                                            </td>
                                            ...
                                            <td class="px-4 py-3 text-sm text-red-700 dark:text-red-400 font-bold">
                        ...
                        <p class="mt-4 text-sm text-gray-500 dark:text-gray-400">Total: {{ $users->count() }} user(s) with expired passwords.</p>
                    @endif
                </div>
            </div>
        </div>
