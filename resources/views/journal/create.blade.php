                    ...
                    <form action="{{ route('journal-entries.store') }}" method="POST" enctype="multipart/form-data" 
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            ...
                            <div>
                                ...
                                <input type="file" name="attachments[]" multiple accept=".pdf,.doc,.docx,.xls,.xlsx,.csv,.jpg,.jpeg,.png"
                                ...
                                <p class="text-xs text-gray-500 mt-1">Accepted: PDF, DOC, XLS, CSV, JPG, PNG (Max 5MB per file)</p>
                            </div>

                            <div class="md:col-span-2">
                                <label class="block text-sm font-bold text-gray-700 mb-2">Description / Reason</label>
                                <textarea name="description" rows="3" required class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full">{{ old('description') }}</textarea>
                            </div>
                        </div>
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-100">
                                        <template x-for="(line, index) in lines" :key="line.id">
                                            <tr :class="{'bg-red-50': line.error}">
                                                <td class="px-4 py-3">
                                                    <select x-model="line.account_id" :name="'lines['+index+'][account_id]'" required class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full text-sm">
                                                ...
                                                <td class="px-4 py-3">
                                                    <select x-model="line.type" :name="'lines['+index+'][type]'" @change="validateSequence()" required class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full text-sm">
                                                ...
                                                <td class="px-4 py-3">
                                                    <div class="relative">
                                                        ...
                                                        <input type="number" step="0.01" min="0.01" required x-model="line.amount" :name="'lines['+index+'][amount]'" class="pl-7 border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full text-sm">
                                                    </div>
                                                </td>
                                                <td class="px-4 py-3">
                                                    <input type="text" x-model="line.description" :name="'lines['+index+'][description]'" placeholder="Optional reference..." class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full text-sm">
                                                </td>
                                                <td class="px-4 py-3 text-center">
                                                    <button type="button" @click="removeLine(line.id)" class="text-red-500 hover:text-red-700 bg-red-50 p-2 rounded-md" x-show="lines.length > 2">
                                    ...
                                    <tfoot class="bg-gray-50">
                                        <tr>
                                            <td colspan="5" class="px-4 py-3 border-b flex items-center space-x-2">
                                                <button type="button" @click="addLine('debit')" class="inline-flex items-center px-3 py-1.5 border border-indigo-200 text-sm font-medium rounded text-indigo-700 bg-indigo-50 hover:bg-indigo-100 mr-2">
                                                    + Add Debit Line
                                                </button>
                                                <button type="button" @click="addLine('credit')" class="inline-flex items-center px-3 py-1.5 border border-amber-200 text-sm font-medium rounded text-amber-700 bg-amber-50 hover:bg-amber-100">
                                                    + Add Credit Line
                                                </button>
                                                
                                                <div class="ml-auto text-sm text-red-600 font-semibold italic flex-grow text-right pr-4" x-show="sequenceError">
                                        ...
                                        <tr>
                                            ...
                                            <td class="px-4 py-4 font-mono font-bold w-48 text-left" :class="isBalanced ? 'text-green-600' : 'text-red-600'">
                                                <div class="flex flex-col">
                                                    <span>Dr: $<span x-text="formatTotal(totalDebits)"></span></span>
                                                    <span>Cr: $<span x-text="formatTotal(totalCredits)"></span></span>
                                                </div>
                                            </td>
                                            <td colspan="2" class="px-4 py-4 text-left font-bold" :class="isBalanced ? 'text-green-600' : 'text-red-600'">
                                                ...
                                                <span x-show="!isBalanced" class="flex items-center">
                                                    <svg class="w-5 h-5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                        ...
                        <div class="flex justify-end pt-5 border-t border-gray-200">
                            <a href="{{ route('journal-entries.index') }}" class="py-2 px-4 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 mr-3">
                                Cancel
                            </a>
                            <button type="button" @click="resetForm" class="py-2 px-4 border border-red-300 text-red-700 rounded-md text-sm font-medium hover:bg-red-50 mr-3">
                                Clear Data
                            </button>
