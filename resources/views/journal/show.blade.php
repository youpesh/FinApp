        ...
        <div class="flex justify-between items-center">
            ...
            @if(Auth::user()->isManager() && $journalEntry->status === 'pending')
                <div class="flex gap-2">
                    <button type="button" onclick="document.getElementById('reject-modal').classList.remove('hidden')" class="bg-red-50 text-red-700 px-4 py-2 border border-red-200 rounded-md hover:bg-red-100 transition shadow-sm font-medium text-sm">
                        Reject Entry
                    </button>
                    <form method="POST" action="{{ route('manager.journal-entries.approve', $journalEntry) }}" class="inline">
                        @csrf
                        <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded-md hover:bg-green-700 transition shadow-sm font-medium text-sm">
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                    ...
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-100 p-6">
                        <div class="flex justify-between border-b pb-4 mb-4">
                            ...
                            <div class="text-right">
                                ...
                                <p class="font-medium text-indigo-600">{{ $journalEntry->creator->full_name ?? 'Unknown User' }}</p>
                            </div>
                        </div>
                                        ...
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-6 py-4">
                                                <div class="text-sm font-medium text-indigo-600 hover:underline">
                                                    <a href="{{ route('accounts.show', $line->account_id) }}">{{ $line->account->account_number }} - {{ $line->account->name }}</a>
                                                </div>
                                            </td>
                                ...
                                @foreach($journalEntry->attachments as $attachment)
                                    <li class="flex items-start bg-gray-50 p-3 rounded-md border border-gray-200 hover:bg-gray-100 transition">
                                        ...
                                        <div class="ml-3 flex-1 overflow-hidden">
                                            <a href="{{ asset('storage/' . $attachment->file_path) }}" target="_blank" class="text-sm font-medium text-indigo-600 truncate block hover:underline" title="{{ $attachment->original_name }}">
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-6 mt-6">
                ...
                <p class="text-sm text-gray-600 mb-4">This transaction has been successfully posted to the General Ledger.</p>
            ...
            <div class="mt-3">
                ...
                <form method="POST" action="{{ route('manager.journal-entries.reject', $journalEntry) }}" class="mt-4">
                    ...
                    <div>
                        ...
                        <textarea name="rejection_reason" rows="3" required class="border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm w-full" placeholder="e.g. Needs more details on invoice..."></textarea>
                    </div>
                    
                    <div class="flex justify-between items-center mt-5 gap-3">
                        <button type="button" onclick="document.getElementById('reject-modal').classList.add('hidden')" class="px-4 py-2 bg-white text-gray-700 border border-gray-300 rounded-md hover:bg-gray-50 w-full transition">
                            Cancel
                        </button>
                        <button type="submit" class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 w-full transition shadow-sm font-medium">
                            Confirm Rejection
                        </button>
                    </div>
                </form>
